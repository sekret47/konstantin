...
{   
	...
	{ 
	  ...
	  $doc->addStyleSheet(JURI::root().'templates/vt_personal/vtemtools/widgets/style.css');
      $doc->addScript(JURI::root().'templates/vt_personal/vtemtools/widgets/jquery-1.7.2.min.js');
	  $doc->addScript(JURI::root().'templates/vt_personal/vtemtools/widgets/script.js');
		...
		foreach ($options as $i => $option) {
			...
			$html[] = '<input type="radio" class="vteminput" id="'.$this->id.$i.'" name="'.$this->name.'"' .
					...
					.$checked.$class.$onclick.$disabled.' title="'.$this->id.'"/>';
