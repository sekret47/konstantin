...
$canOrder	= $user->authorise('core.edit.state', 'com_easy_youtube_gallery');
$saveOrder	= $listOrder == 'a.ordering';
?>

<form action="<?php echo JRoute::_('index.php?option=com_easy_youtube_gallery&view=videos'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EASY_YOUTUBE_GALLERY_VIDEOS_TITLE', 'a.title', $listDirn, $listOrder); ?>
				</th>
				
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EASY_YOUTUBE_GALLERY_VIDEOS_THUMB', 'a.thumb', $listDirn, $listOrder); ?>
				</th>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			$canCreate	= $user->authorise('core.create',		'com_easy_youtube_gallery');
			$canEdit	= $user->authorise('core.edit',			'com_easy_youtube_gallery');
			$canCheckin	= $user->authorise('core.manage',		'com_easy_youtube_gallery');
			$canChange	= $user->authorise('core.edit.state',	'com_easy_youtube_gallery');
			?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_easy_youtube_gallery&task=video.edit&id='.(int) $item->id); ?>">
				...
				<td>
					<?php
						if($item->thumb=="")
						{
							...
							preg_match($regex,$item->url,$ytkey);
							
							echo '<img src="http://img.youtube.com/vi/'.$ytkey[2].'/'.$item->youtubethumb.'.jpg" width=100 />';
						}
						else 
							echo '<img src="'.JURI::root().$item->thumb.'" width=100 />';
						 ?>
				</td>
